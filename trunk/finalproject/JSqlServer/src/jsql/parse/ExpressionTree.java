...
public class ExpressionTree implements Exp {
	...
	public boolean filterByExpression(RowInfo rowInfo, Database database) throws Exception {
		Constant c = evaluate(rowInfo, database);
		if (!(c instanceof BooleanConstant)) throw new Exception("where syntax error!");
		return (Boolean) ((BooleanConstant)c).getBaseValue();
	}
	private Constant evaluate(RowInfo rowInfo, Database database) throws Exception {
		if (isOneChild()) {
			Exp e = getOneChild();
		...
		else if (eL!=null) cL = ((ExpressionTree)eL).evaluate(rowInfo, database);

		Exp eR = getChildRight();
		Constant cR = null;
		if (eR instanceof Constant) cR = (Constant) eR;
		else if (eR!=null) cR = ((ExpressionTree)eR).evaluate(rowInfo, database);
		...
		if (cL instanceof ColumnConstant) {
			...
			Type type = (Type) rowInfo.getAttValue(colName);
